defmodule FootballratingsWeb.UsersAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, FootballratingsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_users/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      FootballratingsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount: :redirect_if_users_is_authenticated" do
    ...
    test "Don't redirect is there is no authenticated users", %{conn: conn} do
      session = conn |> get_session()
