defmodule FootballratingsWeb.UsersAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_footballratings_web_users_remember_me"
  ...
  def log_out_users(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      FootballratingsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_users:
      ...
      defmodule FootballratingsWeb.PageLive do
        use FootballratingsWeb, :live_view

        on_mount {FootballratingsWeb.UsersAuth, :mount_current_users}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FootballratingsWeb.UsersAuth, :ensure_authenticated}] do
        live "/profile", ProfileLive, :index
      end
  """
  def on_mount(:mount_current_users, _params, session, socket) do
    {:cont, mount_current_users(session, socket)}
  end
